...
def test_answer_with_llm_dropdown_selection(monkeypatch):
    ...
    # Mock complete to return "Indonesian Citizen"
    monkeypatch.setattr("src.apply.complete", lambda messages, cfg, **kwargs: "Indonesian Citizen")
...
def test_answer_with_llm_fallback_matching(monkeypatch):
    ...
    monkeypatch.setattr("src.apply.complete", lambda messages, cfg, **kwargs: "1 year")
    ...
    assert ans == "1-2 years" or ans == "Less than 1 year"  # matches partial


def test_fill_known_fields_uses_llm_for_unseen_questions(monkeypatch):
    mock_el = MagicMock()
    mock_el.is_visible.return_value = True
    mock_el.evaluate.side_effect = lambda script: "Notice Period" if "getAttribute" in script else "input"
    ...
    monkeypatch.setattr("src.apply.complete", lambda messages, cfg, **kwargs: "Immediately")
    ...
    mock_el.fill.assert_called_once_with("Immediately")
